...
export function valideerFactuur(factuur: FactuurData): VeldFouten {
  ...
  if (factuur.factuurdatum) {
    if (!isGeldigeDatum(factuur.factuurdatum)) {
  ...
  const vervaldatumFout = controleerVervaldatum(factuur.vervaldatum, factuur.factuurdatum);
  if (vervaldatumFout) fouten["vervaldatum"] = vervaldatumFout;
  ...
  // Leveranciersgegevens: IBAN (lengte per land + mod 97), btw-nummer, KvK-nummer
  const ibanFout = controleerIban(factuur.iban);
  if (ibanFout) fouten["iban"] = ibanFout;
  const btwFout = controleerBtwNummer(factuur.btw_nummer);
  if (btwFout) fouten["btw_nummer"] = btwFout;
  const kvkFout = controleerKvkNummer(factuur.kvk_nummer);
  if (kvkFout) fouten["kvk_nummer"] = kvkFout;
